...
async def handler(event):
    ...
    if msg.reply_to_msg_id:
        print(f"💬 ES UNA RESPUESTA (reply_to_msg_id: {msg.reply_to_msg_id})")
    print("="*70)
    print(texto_mensaje[:1000])
    print("="*70)

    all_ccs = re.findall(r'\d{14,16}\|\d{1,2}\|\d{2,4}\|\d{3,4}', texto_mensaje)
    ...
    if is_mass:
        ...
        gateway = extract_gateway(texto_mensaje)
        print(f"🚪 Gateway detectado: {gateway}")
        
        bank = get_field_flexible(texto_mensaje, ["BANK", "BANCO", "Banco"])
        if bank != "Not Found":
            bank = bank.upper()
        print(f"🏦 Bank: {bank}")
        
        country = get_field_flexible(texto_mensaje, ["COUNTRY", "PAIS", "Pais"])
        flag = "❓"
        if country != "Not Found":
            flag_match = re.search(r'([\U0001F1E6-\U0001F1FF]+)', country)
            if flag_match:
        ...
        info_field = get_field_flexible(texto_mensaje, ["BIN INFO", "INFO", "Data", "Info"])
        if info_field != "Not Found":
            info_field = info_field.upper().strip()
        print(f"💳 Info: {info_field}")
        
        mass_cards = extract_mass_cards(texto_mensaje)
    ...
    card_data = extract_card_info(texto_mensaje)
    if not card_data:
